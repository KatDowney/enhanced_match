...
viewed_did_lat <- vrm_example_map$viewed_retailer_postcode_1
viewed_did_lon <- vrm_example_map$viewed_retailer_postcode_2
sold_did_lat <- vrm_example_map$sold_did_lat
sold_did_lon <- vrm_example_map$sold_did_lon
buyer_lat <- vrm_example_map$buyer_lat
buyer_lon <- vrm_example_map$buyer_lon
...
ggplot() + 
  ...
  coord_fixed(ratio = 1.3, 
              ...
              ylim = c(50, 59)) + 
  theme_void() + 
  geom_point(data = vrm_example_map, aes(x = sold_did_lon, y = sold_did_lat, size = 4, shape = 1, fill = "darkred")) +
  geom_point(data = vrm_example_map, aes(x = viewed_retailer_postcode_2, y = viewed_retailer_postcode_1, size = 1, shape = 23, fill = "blue")) + 
  geom_point(data = vrm_example_map, aes(x = buyer_lon, y = buyer_lat, size = 1, shape = 23, fill = "green"))
...
ggplot(distance_matches_m_agg,aes(x = sold_month,y = value)) + 
    geom_bar(aes(fill = viewed_distance_banding),stat = "identity",position = "fill") + theme_minimal() + scale_fill_brewer() + labs(title = "% of sales by vehicle banding") + labs(x = expression("Month"), y = "Influence") + geom_text(aes(label=distance_matches_m_agg$value, y=distance_matches_m_agg$value/100))
